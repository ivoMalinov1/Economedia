...
resource "google_organization_iam_member" "network_admins" {
  count  = local.network_admins_group != null && local.parent_folder == "" ? 1 : 0
  org_id = local.org_id
  role   = "roles/compute.networkAdmin"
  member = "group:${local.network_admins_group}"
}

resource "google_folder_iam_member" "network_admins" {
  count  = local.network_admins_group != null && local.parent_folder != "" ? 1 : 0
  folder = "folders/${local.parent_folder}"
  role   = "roles/compute.networkAdmin"
  member = "group:${local.network_admins_group}"
...
resource "google_organization_iam_member" "gcp_data_members" {
  count  = local.gcp_data_members != null && local.parent_folder == "" ? 1 : 0
  org_id = local.org_id
  role   = "roles/viewer"
  member = "group:${local.gcp_data_members}"
}

resource "google_folder_iam_member" "gcp_data_members" {
  count  = local.gcp_data_members != null && local.parent_folder != "" ? 1 : 0
  folder = "folders/${local.parent_folder}"
  role   = "roles/viewer"
  member = "group:${local.gcp_data_members}"
...
resource "google_folder_iam_member" "gcp_analytics_members" {
  count  = local.gcp_analytics_members != null && local.parent_folder != "" ? 1 : 0
  folder = "folders/${local.parent_folder}"
  role   = "roles/viewer"
  member = "group:${local.gcp_analytics_members}"
}

resource "google_organization_iam_member" "gcp_analytics_members" {
  count  = local.gcp_analytics_members != null && local.parent_folder == "" ? 1 : 0
  org_id = local.org_id
  role   = "roles/viewer"
  member = "group:${local.gcp_analytics_members}"
